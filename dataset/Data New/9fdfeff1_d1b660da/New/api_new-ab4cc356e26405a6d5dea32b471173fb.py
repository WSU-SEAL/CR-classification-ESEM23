...
class API(base.Base):
    ...
    def _create_instance(self, context, instance_type,
        ...
        # Set validate_numa=False since numa validation is already done
    ...
    def resize(self, context, instance, flavor_id=None, clean_shutdown=True,
        ...
        image = obj_base.obj_to_primitive(request_spec.image)
        image = jsonutils.loads(jsonutils.dumps(image))
        bdms = objects.BlockDeviceMappingList.get_by_instance_uuid(
            context, instance.uuid)
        root_bdm = compute_utils.get_root_bdm(context, instance, bdms)
        self._validate_flavor_image(context, image.get('id', ''), image,
