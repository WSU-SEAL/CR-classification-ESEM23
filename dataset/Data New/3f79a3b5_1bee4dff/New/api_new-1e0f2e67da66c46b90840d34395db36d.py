...
"""),
    cfg.BoolOpt("local_metadata_per_cell",
                default=False,
                help="""
Indicating that the metadata-api service has been deployed per-cell, so that
we can avoid query API DB for instance information. Users should consider the
use of this configuration depending on how neutron is setup, you have 
networks that span cells, you might need to run nova-metadata-api global, if
...
should also correctly config each neutron metadata-agent to point to the
