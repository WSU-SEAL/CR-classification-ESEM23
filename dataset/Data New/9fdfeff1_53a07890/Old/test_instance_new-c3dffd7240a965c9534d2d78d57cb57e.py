...
class TestInstanceNotificationSample(
    ...
    def test_volume_swap_server_with_error(self):
        ...
        # Eight versioned notifications are generated. We only rely on the
        # first seven because _wait_until_swap_volume_error will return True
        ...
        # Per the note above, we could race to get the compute.exception
        # notification so we need to wait for it here.
        for x in range(10):
            if len(fake_notifier.VERSIONED_NOTIFICATIONS) == 8:
                break
            time.sleep(0.5)
        else:
            self.fail('Timed out waiting for final versioned notification. '
                      'Got: %s' % fake_notifier.VERSIONED_NOTIFICATIONS)
