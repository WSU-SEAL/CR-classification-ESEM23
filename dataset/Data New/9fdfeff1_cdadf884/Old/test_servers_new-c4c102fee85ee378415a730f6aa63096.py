...
class PortResourceRequestBasedSchedulingTestIgnoreMicroversionCheck(
    ...
    # Mocking detach is a hack but the fake virt driver does not allow
    # detaching interfaces that are added during the boot. Also the fake virt
    # driver is not easy to fix to allow it. In the other hand ports with
    # bandwidth request cannot be attached after the server is booted.
    @mock.patch('nova.virt.fake.SmallFakeDriver.detach_interface')
    def test_interface_detach_with_port_with_bandwidth_request(
            self, mock_detach):
        # create a server
        server = self._create_server(
            flavor=self.flavor,
