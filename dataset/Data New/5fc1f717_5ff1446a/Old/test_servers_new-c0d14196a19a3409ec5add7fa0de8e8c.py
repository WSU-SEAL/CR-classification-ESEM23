...
class TraitsBasedSchedulingTest(integrated_helpers.ProviderUsageBaseTestCase):
    ...
    def test_flavor_traits_based_scheduling(self):
        ...
        ends up in ERROR state when the resource provider with the required
        trait in placement is disabled.
        """
    ...
    def test_flavor_forbidden_traits_based_scheduling(self):
        """Tests that a server create request using a forbidden trait on flavor
        ends up in ERROR state when the resource provider without the
        forbidden trait in placement is disabled.
        """

        # Decorate compute1 resource provider with the forbidden trait.
        rp_uuid = self._get_provider_uuid_by_host(self.compute1.host)
        self._set_provider_traits(rp_uuid, ['HW_CPU_X86_SGX'])

        # Disable the compute node that does not have the forbidden trait
        compute2_service_id = self.admin_api.get_services(
            host=self.compute2.host, binary='nova-compute')[0]['id']
        self.admin_api.put_service(compute2_service_id, {'status': 'disabled'})

        # Create server using only flavor forbidden trait
