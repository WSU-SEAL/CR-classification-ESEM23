    ...
    cfg.StrOpt('sysinfo_serial',
               ...
               choices=(
                   ('none', 'A serial number entry is not added to the guest '
                            'domain xml. This will also affect existing '
                            'instances on this host once they stop and start '
                            'again.'),
                   ...
                   ('hardware', 'A UUID for the host hardware as reported by '
                                ...
                                'in ``libvirtd.conf``. This will also affect '
                                'existing instances on this host once they '
                                'stop and start again.'),
                   ('auto', 'Uses the "os" source if possible, else '
                            '"hardware". This will also affect existing '
                            'instances on this host once they stop and start '
                            'again.'),
               ...
               deprecated_reason="""
               Applications that depend on the serial number within the 
               guest were not guaranteed it would be unique or not changed 
               during migrations if options other than ``unique`` is set. 
               This config option will be removed in Train release and all 
               instances will use ``instance_uuid`` as serial number from 
               then on.
               """,
               ),
