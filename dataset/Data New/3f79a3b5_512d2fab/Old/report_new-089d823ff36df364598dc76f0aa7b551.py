        ...
        if r.status_code != 204:
            ...
            if err['code'] == 'placement.concurrent_update':
                ...
                if 'consumer generation conflict' in err['detail']:
                    # NOTE(mriedem): Deal with a race where the consumer
                    # was deleted between the time we got the allocations with
                    # a consumer_generation and we PUT the allocations using
                    # that consumer_generation, and placement will create a
                    # new consumer record with a null generation and that
                    # causes the conflict.
                    if 'expected null but got' in err['detail']:
                        # In this case, we simply retry because the
                        # GET /allocations/{consumer_id} should be empty
                        # and we'll be oh so happy.
                        reason = ('another process removed the consumer '
                                  'involved in our attempt to put allocations '
                                  'for consumer %s' % consumer_uuid)
