...
class API(base_api.NetworkAPI):
    ...
    def _unbind_ports(self, context, ports,
        ...
        for port_id in ports:
            ...
            network = neutron.show_network(port.get('network_id'),
                                           fields='dns_domain').get('network')
            # NOTE: for internal DNS integration, we use the admin client to
            # reset the dns_name
            ...
            try:
                ...
                LOG.exception("Unable to clear device ID for port '%s'",
                              port_id)
            # NOTE: for external DNS integration, we use the neutron client with
            # user's context to reset the dns_name since the recordset is under
            # user's zone.
    ...
    def _reset_port_dns_name(self, network, port_id, neutron):
