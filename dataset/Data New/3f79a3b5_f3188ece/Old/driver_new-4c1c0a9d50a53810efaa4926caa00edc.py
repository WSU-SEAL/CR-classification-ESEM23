...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _disk_raw_to_qcow2(path):
        ...
        # Use no-caching option when O_DIRECT is supported to reduce
        # interference with other applications and ensure that the data is on
        # persistent storage when the command exits.
        if nova.privsep.utils.supports_direct_io(CONF.instances_path):
            cache_mode = 'none'
        else:
            cache_mode = 'writethrough'
        utils.execute('qemu-img', 'convert', '-f', 'raw', '-t', cache_mode,
