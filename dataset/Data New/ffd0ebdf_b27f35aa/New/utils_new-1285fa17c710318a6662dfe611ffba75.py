...
def get_weight_multiplier(host_state, multiplier_name, multiplier_config):
    """Given a host_state object, multplier_type name and multiplier_config,
    ...
    contain the multiplier_type, the multiplier_config will be returned
    directly.
    ...
    :param multiplier_config: The weight multiplier configuration
    """
    aggregate_vals = utils.aggregate_values_from_key(host_state,
                                                     multiplier_name)
    try:
        value = utils.validate_num_values(
            aggregate_vals, multiplier_config, cast_to=float)
    except ValueError as e:
        LOG.warning("Could not decode weight multiplier: '%s'", e)
