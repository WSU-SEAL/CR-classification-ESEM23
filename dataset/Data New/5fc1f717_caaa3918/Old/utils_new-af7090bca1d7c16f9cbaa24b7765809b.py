...
class ResourceRequest(object):
    ...
    def to_querystring(self):
        ...
        if nr_of_numbered_groups >= 2 and not self._group_policy:
            ...
            LOG.warning(
                "There are more than one numbered request group in the "
                ...
                "(i.e. the port has QoS minimum bandwidth policy rule "
                "attached) then you have to specify the group policy in the "
                "flavor extra_spec. If it is OK to let these groups satisfied "
                "from overlapping resource providers then use "
                "'group_policy': 'None'. If you want that each group is "
