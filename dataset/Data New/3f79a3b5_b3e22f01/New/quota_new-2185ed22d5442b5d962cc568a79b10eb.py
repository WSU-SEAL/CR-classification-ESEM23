...
def _server_group_count_members_by_user(context, group, user_id):
    ...
    for cell_mapping in cell_mappings:
        # TODO(tssurya): Handle unavailable cells below by using the
...
def _instances_cores_ram_count(context, project_id, user_id=None):
    ...
    # this filtering if there is more than one non-cell0 cell. As a part of
    # the handling-down-cell blueprint, we also introduce a policy check
    # for (dis-)allowing server creation if the tenant already has active
    # instances in the down cells.
