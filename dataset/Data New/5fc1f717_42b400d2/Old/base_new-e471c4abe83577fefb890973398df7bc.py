...
class ServersTestBase(base.ServersTestBase):
    ...
    def setUp(self):
        ...
        def fake_noop(*args, **kwargs):
            return None
        self.stub_out('nova.privsep.utils.supports_direct_io', fake_noop)

        # NOTE(stephenfin): Plugging VIFs can have real side-effects so mock
        # these out
        self.stub_out('nova.virt.libvirt.vif.LibvirtGenericVIFDriver.'
                      '_plug_os_vif', fake_noop)
        self.stub_out('nova.virt.libvirt.vif.LibvirtGenericVIFDriver.'
                      '_unplug_os_vif', fake_noop)
