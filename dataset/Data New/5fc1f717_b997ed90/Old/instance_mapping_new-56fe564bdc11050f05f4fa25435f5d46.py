...
def populate_user_id(context, max_count):
    ...
    for cell_id, inst_uuids in inst_uuids_by_cell_id.items():
        ...
        if cell_id is None:
            LOG.debug('Skipping migration of instance mappings %s because '
                      'they do not have a cell.', inst_uuids)
            continue
        with nova_context.target_cell(context, cms_by_id[cell_id]) as cctxt:
            ...
            try:
                ...
                LOG.debug('Encountered exception: "%s" while querying '
                          'instances from cell: %d. Continuing to the next '
                          'cell.', six.text_type(exp), cell_id)
                continue
        ...
        if ims_by_inst_uuid:
            LOG.debug('Skipping migration of instance mappings %s because '
                      'they do not have instances.', ims_by_inst_uuid.keys())
            if not orphaned_ims_warned:
                LOG.warning('Orphaned instance mappings were found. They must '
                            'be cleaned up/removed manually before this '
                            'online data migration can complete.')
                orphaned_ims_warned = True
        if done >= max_count:
            break
