...
def _get_node_empty_ratio(context, max_count):
    empty_ratio = ['0.0', None]
    return (context.session.query(models.ComputeNode)).filter(or_(
        models.ComputeNode.ram_allocation_ratio.in_(empty_ratio),
        models.ComputeNode.cpu_allocation_ratio.in_(empty_ratio),
        models.ComputeNode.disk_allocation_ratio.in_(empty_ratio)
    )).limit(max_count).all()


@db_api.pick_context_manager_writer
def refresh_empty_ratio(context, max_count):
    cns = _get_node_empty_ratio(context, max_count)

    # NOTE(yikun): If it's an existing record with 0.0 or None values,
    # we need to do what the compute does, which is use the configure
    # 'xxx_allocation_ratio' confi if it's not None, and fallback to
    # using the 'initial_xxx_allocation_ratio' otherwise.
    processed = 0
    for cn in cns:
...
class ComputeNode(base.NovaPersistentObject, base.NovaObject):
    ...
    def _from_db_object(context, compute, db_compute):
        ...
        for key in fields:
            ...
            # allocation ratios.
            # TODO(yikun): Remove that when all ratio values are NOT 0.0 or
            # NULL
