...
class ServersSampleJson270Test(ServersSampleBase):
    microversion = '2.70'
    scenarios = [('v2_70', {'api_major_version': 'v2.1'})]

    def setUp(self):
        super(ServersSampleJson270Test, self).setUp()
        self.common_subs = {
            'hostid': '[a-f0-9]+',
            'instance_name': 'instance-\d{8}',
            'hypervisor_hostname': r'[\w\.\-]+',
    ...
    def stub_get_by_instance_uuid(self, *kargs, **kwargs):
        uuid = "11111111-1111-1111-1111-111111111111"
        return objects.instance_group.InstanceGroup(name='sg1', uuid=uuid)

    def test_servers_post(self):
        return self._post_server(use_common_server_api_samples=True)

    def test_servers_get_with_server_group(self):
        uuid = self.test_servers_post()
        self.stub_out(
            'nova.objects.InstanceGroup.get_by_instance_uuid',
             self.stub_get_by_instance_uuid)
        response = self._do_get('servers/%s' % uuid)
        subs = self.common_subs.copy()
        subs['id'] = uuid
        self._verify_response('server-get-resp', subs, response, 200)
    ...
    def test_servers_rebuild_with_server_groups(self):
        uuid = self._post_server(use_common_server_api_samples=True)
        fakes.stub_out_key_pair_funcs(self)
        self.stub_out(
            'nova.objects.InstanceGroup.get_by_instance_uuid',
             self.stub_get_by_instance_uuid)
