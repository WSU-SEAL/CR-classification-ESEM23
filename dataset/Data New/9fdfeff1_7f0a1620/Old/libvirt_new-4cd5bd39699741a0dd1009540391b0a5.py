...
"""),
    ...
    cfg.StrOpt('sysinfo_serial',
               ...
               help='The data source used to the populate the host "serial" '
                    ...
                    'If the user uses a flavor with extra spec '
                    '``hw:unique_serial=True`` or the corresponding image '
                    'property ``hw_unique_serial=True`` when create '
                    'instance, the guest will overwrite this configuration '
                    'and use ``instance_uuid`` as serial.'),
