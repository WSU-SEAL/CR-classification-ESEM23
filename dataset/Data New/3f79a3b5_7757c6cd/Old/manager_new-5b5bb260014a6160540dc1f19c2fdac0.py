        ...
        for existed in self.pci_devs:
            if existed.address in exist_addrs - new_addrs:
                ...
                try:
                    ...
                    # Note(sean-k-mooney): the device may not exists for
                    # two reasons. first the device could have been been
                    # removed form the host or second the whitelist could
                    # have been updated. While force removing may seam
                    # resonable if the device is inuse by a vm this
                    # will lead to https://bugs.launchpad.net/nova/+bug/163312.
                    # To prevent this bug we skip deleting the device form the
                    # db in this iteration and will try again on the next sync.
                    continue
                else:
