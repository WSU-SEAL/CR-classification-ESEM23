...
class API(base_api.NetworkAPI):
    ...
    def update_instance_nw_info(self, context, instance, migration,
                                network_info):
        if migration.get('status') != 'confirming':
            return network_info
        # In case of migration of VM with pci slots, we need
        # to know previously used pci slots in the source host for proper
        # cleanup. Update network_info, to use old pci slots.
        pci_mapping = self._get_pci_mapping_for_migration(context,
                                                          instance,
                                                          migration)
        if pci_mapping:
            for net in network_info:
                for old_dev, new_dev in pci_mapping.iteritems():
                    if net['profile']['pci_slot'] == new_dev.address:
                        net['profile']['pci_slot'] = old_dev
        return network_info
