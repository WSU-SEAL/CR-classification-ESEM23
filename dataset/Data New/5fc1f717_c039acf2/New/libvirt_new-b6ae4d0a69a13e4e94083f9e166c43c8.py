...
* default: "It Depends" -- For Nova-managed disks, ``none``, if the host
  is capable of it (by having Linux's "O_DIRECT" support on the given
  host file system); otherwise ``writeback``.  For volume drivers, the
  default is driver-dependent: ``none`` for everything except for SMBFS
  and Virtuzzo (which use ``writeback``).
