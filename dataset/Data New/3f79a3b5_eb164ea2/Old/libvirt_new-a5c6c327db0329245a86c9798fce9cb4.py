...
"""),
    cfg.StrOpt('live_migration_timeout_action',
               ...
               mutable=True,
               help="""
This option will determine what actions will be taken against a VM after
``live_migration_completion_timeout`` expires. Currently nova just aborts
the LM operation after completion timeout expires. By default, we keep the
same behavior of aborting after completion timeout. ``force_complete`` will
either pause the VM or trigger post_copy depending on if post copy is
...
least 10% over the last iteration, or will be activated if
``live_migration_timeout_action`` is set to 'force_complete' and we reach the
timeout defined by ``live_migration_completion_timeout``. Note if you change
to not timeout, i.e. ``live_migration_completion_timeout = 0``, then there
will be no automatic switch to post-copy.
