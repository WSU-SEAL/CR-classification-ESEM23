...
class InstanceMappingTestCase(test.NoDBTestCase):
    ...
    def test_populate_user_id(self):
        ...
        # Create a soft-deleted instance with a user_id=None instance mapping.
        ...
        done, total = instance_mapping.populate_user_id(self.context, 2)
        # Two needed fixing, and honored the limit
        self.assertEqual(2, done)
        self.assertEqual(2, total)

        done, total = instance_mapping.populate_user_id(self.context, 1000)
        # Only three left that needed fixing
        self.assertEqual(3, done)
        self.assertEqual(3, total)

        # Check that we have only the expected number of records with
        # user_id set. We created 10 instances (5 per cell with 2 per cell
        # with NULL user_id), 1 soft-deleted instance with NULL user_id,
        # 1 deleted instance with NULL user_id, and 1 not-yet-scheduled
        # instance with NULL user_id.
        # We expect 11 of them to have user_id set after migration (13 total,
        # with the deleted instance and not-yet-scheduled instance ignored).
        mappings = instance_mapping.InstanceMappingList.get_by_project_id(
            self.context, self.context.project_id)
        self.assertEqual(11, len(
            [im for im in mappings if im.user_id is not None]))

        # Check that one instance mapping record (not yet scheduled) has not
        # been migrated by this script. If the instance began creation on the
        # old side of an upgrade and became scheduled on the new side of the
        # upgrade, the active migration in conductor will populate instance
        # mapping user_id.
        # Check that one other instance mapping record (deleted instance) has
        # not been migrated by this script.
