...
@db_api.api_context_manager.reader
def user_id_populated(context, project_id=None):
    """Determine whether user_id is set for non-deleted instances.
    ...
    instance mappings for the instance count).

    Note that this check specifies queued_for_deleted=False, which excludes
    deleted and soft-deleted instances. The 'populate_user_id' data migration
    migrates soft-deleted instances because they could be restored at any time
    in the future. However, for this quota-check-time method, it is acceptable
    to ignore soft-deleted instances, since we just want to know if it is safe
    to use instance mappings to count instances at this point in time (and
    soft-deleted instances do not count against quota limits).

    :param project_id: The project to check
    :returns: True if user_id is set for all non-deleted instances, else False
    """
...
def populate_user_id(context, max_count):
    ...
    for cell in cells:
        ims = (
            ...
            # user_id populated. We need to include records with
            # queued_for_delete=True because they include soft-deleted
            # instances, which could be restored at any time in the future.
            # If we don't migrate soft-deleted instances now, we wouldn't
            # be able to retire this migration code later.
            context.session.query(api_models.InstanceMapping)
            .options(joinedload('cell_mapping'))
            .filter_by(user_id=None)
        ...
        ims_by_inst = {im.instance_uuid: im for im in ims}
        with nova_context.target_cell(context, cell) as cctxt:
            # We need to migrate soft-deleted instances because they could be
            ...
            # NOTE: it's not possible to query only for soft-deleted instances.
            # We must query for both deleted and soft-deleted instances.
            filters = {'uuid': list(ims_by_inst.keys())}
            instances = objects.InstanceList.get_by_filters(
                cctxt, filters, expected_attrs=[])
        ...
        for instance in instances:
            ...
            if instance.user_id is None:
                continue
            # Skip any 'deleted' instance because they cannot be restored (only
            # soft-deleted instances can be restored).
            if instance.deleted:
                continue
