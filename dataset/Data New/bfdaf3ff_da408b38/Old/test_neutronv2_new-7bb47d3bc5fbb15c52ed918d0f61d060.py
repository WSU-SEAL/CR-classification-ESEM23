...
class TestNeutronv2WithMock(_TestNeutronv2Common):
    ...
    def test_update_instance_nw_info(self):
        instance = fake_instance.fake_instance_obj(self.context)
        instance.migration_context = objects.MigrationContext()
        old_pci_devices = objects.PciDeviceList(
            objects=[objects.PciDevice(vendor_id='1377',
        ...
        network_info = [{"profile": {"pci_slot": "0000:0b:00.1",
            "physical_network":"providerInternal",
            "capabilities": ["switchdev"]}}] 

        net_info = self.api.update_instance_nw_info(
            self.context, instance, migration, network_info)
        updated_net_info = [{"profile": {"pci_slot": "0000:0a:00.1",
            "physical_network":"providerInternal",
            "capabilities": ["switchdev"]}}]
        self.assertEqual(updated_net_info, net_info)
