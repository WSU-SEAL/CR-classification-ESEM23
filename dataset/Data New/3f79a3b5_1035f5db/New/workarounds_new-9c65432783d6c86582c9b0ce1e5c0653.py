...
during cleanup on hosts using the RBD imagebackend. This avoids the
following bugs with evacuation and revert resize clean up that lead to the
instance directory remaining on the host that in turn will result in
DestinationDiskExists errors being raised if the instances ever attempt to
return to the same host:
