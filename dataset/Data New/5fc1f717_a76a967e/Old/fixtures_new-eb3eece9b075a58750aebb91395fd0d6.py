...
class SleepPoisonFixture(fixtures.Fixture):
    def setUp(self):
        super(SleepPoisonFixture, self).setUp()
        # Enable tests that really want to sleep, but force them to be explicit
        # about it
        ...
        def _sleep(seconds):
            if seconds != 0:
                ...
                # TODO(efried): Figure out how to mock the sleep in
                # oslo_messaging._drivers.impl_fake.FakeListener.poll
                if 'oslo_messaging/_drivers/impl_fake.py' not in tb:
                    msg = (
                        "Your test, or code it invokes, is calling "
                        "time.sleep(). In order to reduce test time, the "
                        "SleepPoisonFixture prevents time.sleep() from being "
                        "called directly. If you really need to sleep, please "
                        "call self.real_sleep(). Otherwise, please mock out "
                        "time.sleep() in this test case.")
                    raise ValueError('\n'.join((tb, msg)))
            return self.real_sleep(seconds)
        self.useFixture(fixtures.MockPatch('time.sleep', side_effect=_sleep))
