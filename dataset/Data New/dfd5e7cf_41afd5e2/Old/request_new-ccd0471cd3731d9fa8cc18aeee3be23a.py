...
from nova.objects import compute_node as compute_node_objs
...
def get_instance_pci_request_from_vif(context, instance, vif):
    ...
    if vif_pci_dev_address:
        ...
        inst_compute_node_id = inst_compute_node.id
        vif_pci_dev = None
        for pci_dev in instance.pci_devices:
            if pci_dev.compute_node_id == inst_compute_node_id and \
                    pci_dev.address == vif_pci_dev_address:
                vif_pci_dev = pci_dev
                break
        if vif_pci_dev:
        ...
        if vif_pci_request is None:
            LOG.error("Failed to locate PCI request associated with"
                      ...
                      vif_pci_dev_address, inst_compute_node_id)
