...
class SchedulerReportClient(object):
    ...
    def ensure_resource_provider(self, context, uuid, name=None,
        ...
        # We either don't have it locally, or it's stale. Pull it or create it.
        created_rp = None
    ...
    def _associations_stale(self, uuid):
        ...
        Always False if refresh has been disabled by setting
        CONF.compute.resource_provider_association_refresh to zero.
        """
    ...
    def _update_inventory_attempt(self, context, rp_uuid, inv_data):
        ...
        if result.status_code == 409:
            ...
            # ensure_resource_provider() because we know the resource provider
            # record already exists. We're just reloading the record here.
            self.ensure_resource_provider(context, rp_uuid)
            return False
        elif not result:
            placement_req_id = get_placement_request_id(result)
    ...
    def get_provider_tree(self, context):
        ...
        """
        # Ensure all cached RPs are up to date
        for uuid in self._provider_tree.get_provider_uuids():
            if self._associations_stale(uuid):
                self._refresh_and_get_inventory(context, uuid)
                self._refresh_associations(context, uuid, force=False)
    ...
    def _set_inventory_for_provider(self, context, rp_uuid, inv_data):
        ...
        - We don't ensure_resource_provider or refresh_and_get_inventory,
    ...
    def update_compute_node(self, context, compute_node):
        ...
        :raises `exception.InventoryInUse` if the compute node has had changes
                ...
                placement API.
        """
        self.ensure_resource_provider(context, compute_node.uuid,
    ...
    def update_from_provider_tree(self, context, new_tree, allocations=None):
        ...
        def catch_all(rp_uuid):
            ...
            try:
                ...
                # Invalidate the cache. It gets rebuilt on the next pass.
