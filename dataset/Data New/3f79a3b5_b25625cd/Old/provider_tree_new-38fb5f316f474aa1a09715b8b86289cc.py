...
class ProviderTree(object):
    ...
    def remove_tree(self, name_or_uuid):
        """Safely removes the provider identified by the supplied name_or_uuid,
        as well as all of its descendants *and* ancestors. Compare and contrast
        with the remove method, which only removes the subtree starting at
        name_or_uuid.

        :raises ValueError if name_or_uuid points to a non-existing provider.
        :param name_or_uuid: Either name or UUID of the resource provider to
                             remove from the tree.
        :return: List of UUIDs of providers that were deleted.
        """
        with self.lock:
            found = self._find_with_lock(name_or_uuid)
            while found.parent_uuid:
                found = self._find_with_lock(found.parent_uuid)
            ret = found.get_provider_uuids()
            self._remove_with_lock(found.uuid)
            return ret
