...
"""),
    cfg.BoolOpt("local_metadata_per_cell",
                default=False,
                help="""
Indicates that the metadata-api service has been deployed per-cell, so that
we can avoid querying API DB for instance information. Users should consider
the use of this configuration depending on how neutron is setup. If you have
networks that span cells, you might need to run nova-metadata-api globally. If
your networks are segmented along cell boundaries, then you can run
nova-metadata-api per cell. When running nova-metadata-api per cell, you
should also correctly configure each Neutron metadata-agent to point to the
corresponding nova-metadata-api.
"""),
]
