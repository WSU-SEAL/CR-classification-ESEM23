...
class API(base_api.NetworkAPI):
    ...
    def _is_valid_port_rp_mapping(
            self, mapping, ports, placement_allocations, provider_traits):
        # Check that traits are matching for each port - rp pair in
        # this mapping
        for port_id, rp_uuid in mapping:
            required_traits = ports[port_id]['resource_request']['required']
            # NOTE(gibi): Here we assume that neutron only uses required traits
            # and does not use forbidden traits
            if (not all(req_trait in provider_traits[rp_uuid]
                        for req_trait in required_traits)):
                return False

        # Check that ports can consume the requested resources from the rp
        # that it is mapped to in the current mapping. Consume each port's
        ...
        for port_id, rp_uuid in mapping:
            ...
            port_request = ports[port_id]['resource_request']['resources']
    ...
    def _get_resource_providers_for_ports(
            self, ports, placement_allocations, provider_traits):
        """Return the resource provider for each port resource request.

        The mapping is generated based on the overall allocation made for the
        instance, each port's resource request, and the traits of the RPs in
        the allocation.

        This is a workaround as placement does not return which PR fulfills
        ...
        :param ports: dict keyed by port_id containing the port body as
                      returned by neutron show_port
        :param placement_allocations: The overall allocation made by the
                                      scheduler based on the port's resource
                                      request
        ...
        :return: A dict with port_id: resource_provider_uuid mappings for each
                 port that has resource request. Or raises an exception if the
                 mapping is ambiguous.
        """

        # Remove ports that does not require resources
        # We use OrderedDict just to keep the algorithm deterministic
        ports_with_resource_request = collections.OrderedDict(
            (uuid, port) for uuid, port in ports.items()
            if 'resource_request' in port)

        # Nothing left to do, so let's return early
        if not ports_with_resource_request:
            return {}

        # Iterate through every possible port - RP mappings and try to find a
        ...
        # Neutron perspective.
        LOG.debug('Trying to find a valid port - RP mapping for ports %s to '
                  'allocations %s with traits %s', ports_with_resource_request,
                  placement_allocations, provider_traits)

        # This generator first creates permutations with repetition of the RPs
        # with length of the number of ports we have. So if there is
        #   2 RPs (rp1, rp2) and
        #   3 ports (p1, p2, p3).
        ...
        #  ...
        #  (rp2, rp2, rp2)
        # Then we zip each of this permutations to our port list resulting in a
        # list of list of port - rp pairs:
        # [[('p1', 'rp1'), ('p2', 'rp1'), ('p3', 'rp1')],
        #  [('p1', 'rp1'), ('p2', 'rp1'), ('p3', 'rp2')],
        #  [('p1', 'rp1'), ('p2', 'rp2'), ('p3', 'rp1')],
        #  ...
        #  [('p1', 'rp2'), ('p2', 'rp2'), ('p3', 'rp2')]]
        ...
        every_possible_mapping = (list(zip(ports_with_resource_request, rps))
                                  for rps in itertools.product(
                                      placement_allocations.keys(),
                                      repeat=len(ports)))
        for mapping in every_possible_mapping:
            if self._is_valid_port_rp_mapping(
                    mapping, ports_with_resource_request,
                    placement_allocations, provider_traits):
                LOG.debug('Found valid port - RP mapping %s', list(mapping))
                return dict(mapping)
        ...
        # including the port's request as well.
        raise ValueError('No valid port - RP mapping is found for '
                         'ports %s, allocation %s and provider traits %s' %
                         (ports, placement_allocations, provider_traits))
