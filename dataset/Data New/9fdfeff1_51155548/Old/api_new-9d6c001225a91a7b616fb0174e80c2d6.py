...
class API(base.Base):
    ...
    def restore(self, context, instance):
        ...
        if instance.host:
            ...
            instance.deleted_at = None
            instance.save(expected_task_state=[None])
        im = self._update_queued_for_deletion(context, instance, False)
        # Soft-deleted instances were not included in the data migration to
        # populate user_id, so set it if it's NULL.
        if 'user_id' not in im or im.user_id is None:
