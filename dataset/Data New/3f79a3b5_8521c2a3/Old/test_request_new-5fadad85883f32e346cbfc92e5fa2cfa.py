...
class PciRequestTestCase(test.NoDBTestCase):
    ...
    @staticmethod
    def _create_fake_context():
        return context.RequestContext(fakes.FAKE_USER_ID,
    ...
    def test_get_instance_pci_request_from_vif(self, cn_get_by_host_and_node):
        inst_cn = mock.Mock()
        cn_get_by_host_and_node.return_value = inst_cn
        inst_cn.id = 1
        ...
        inst = PciRequestTestCase._create_fake_inst_with_pci_devs(
            pci_request_list,
            pci_device_list)
        context = PciRequestTestCase._create_fake_context()
