...
class CheckRequestedFlavorTestCase(test.TestCase):
    def setUp(self):
        super(CheckRequestedFlavorTestCase, self).setUp()
        self.compute_api = compute.API()
        ...
        self.image = dict(id=uuids.image_id, status='foo')
    ...
    def test_pci_validated(self, mock_request):
        self.compute_api._validate_flavor_image_nostatus(self.context,
                self.image, self.instance_type, None, validate_pci=True)
        mock_request.assert_called_once_with(self.instance_type)
