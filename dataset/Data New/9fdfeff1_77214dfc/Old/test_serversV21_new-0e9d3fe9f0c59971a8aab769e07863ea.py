...
class ServersControllerTestV270(ControllerTest):
    ...
    @mock.patch.object(InstanceGroup, 'get_by_instance_uuid',
                       return_value=InstanceGroup(name='sg1', uuid=FAKE_UUID))
    def test_show_with_server_group(self, mock_sg_get):
        req = self.req('/fake/servers/%s' % FAKE_UUID)
        res_dict = self.controller.show(req, FAKE_UUID)
...
class ServersControllerRebuildTestV270(ControllerTest):
    ...
    @mock.patch.object(InstanceGroup, 'get_by_instance_uuid',
                       return_value=InstanceGroup(name='sg1', uuid=FAKE_UUID))
    def test_rebuild_with_server_group(self, mock_sg_get):
        self.req.body = jsonutils.dump_as_bytes(self.body)
...
class ServersControllerUpdateTestV270(ServersControllerUpdateTest):
    ...
    @mock.patch.object(InstanceGroup, 'get_by_instance_uuid',
                       return_value=InstanceGroup(name='sg1', uuid=FAKE_UUID))
    def test_update_with_server_group(self, mock_sg_get):
        body = {'server': {
                  'name': 'server_test',
               }}
        req = self._get_request(body)
