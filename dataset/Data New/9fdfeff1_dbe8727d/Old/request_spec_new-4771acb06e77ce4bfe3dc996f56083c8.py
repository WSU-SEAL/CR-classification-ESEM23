...
class RequestSpec(base.NovaObject):
    ...
    def _from_db_object(context, spec, db_spec):
        ...
        if 'ignore_hosts' in spec and spec.ignore_hosts:
            spec.ignore_hosts = []
    ...
    def _get_update_primitives(self):
        ...
        if updates:
            ...
            # NOTE(sean-k-mooney): Don't persist ignore_host to the db as
            # this could cause errors if we ever modify and save a request
            # spec during a live migration or other move operations and
            # forget to reset changes to this field.
            if 'ignore_hosts' in spec and spec.ignore_hosts:
                del spec.ignore_hosts
