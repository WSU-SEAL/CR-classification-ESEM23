...
# Note any modules with known monkey-patching issues which have been imported
# before monkey patching.
# urllib3: https://bugs.launchpad.net/nova/+bug/1808951
# oslo_context.context: https://bugs.launchpad.net/nova/+bug/1773102
problems = set(['urllib3', 'oslo_context.context']) & set(sys.modules.keys())
...
# before monkey patching.
# NOTE(mdbooth): Ideally we would raise an exception here, as this is likely to
# cause problems when executing nova code. However, some non-nova tools load
# nova only to extract metadata and do not execute it. Two examples are
# oslopolicy-policy-generator and sphinx, both of which fail if we assert here.
# It is not ideal that these utilities are monkey patching at all, but as they
# have always done it and seem to get away with it, we should not break it. If
# there is any way to reliably determine if we are being loaded in that kind of
# context without breaking existing callers, we should do it and bypass monkey
# patching here entirely. I cannot think of one, though.
if problems:
    from oslo_log import log as logging

    LOG = logging.getLogger(__name__)
    LOG.warning("Modules with known eventlet monkey patching issues were "
                "imported prior to eventlet monkey patching: %s. This warning "
                "can usually be ignored if the caller is only importing and "
                "not executing nova code.", ', '.join(problems))
