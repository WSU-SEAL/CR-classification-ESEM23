...
if (os.environ.get('OS_NOVA_DISABLE_EVENTLET_PATCHING', '').lower()
    ...
    # oslopolicy-policy-generator and sphinx, both of which can fail if we assert
    # here. It is not ideal that these utilities are monkey patching at all, but we
    # should not break them.
    # TODO(mdbooth): If there is any way to reliably determine if we are being
    # loaded in that kind of context without breaking existing callers, we should
    # do it and bypass monkey patching here entirely.
    if problems:
