...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def cleanup(self, context, instance, network_info, block_device_info=None,
        ...
        # when using the shared RBD imagebackend. This workaround is not
        # required when cleaning up migrations that provide migrate_data to
        # this method as the existing is_shared_block_storage conditional will
        # cause the instance directory to be removed correctly.  Note however
        # that this does not resolve bug #1761062 that applies to the cleanup
        # of the instance directory during a revert resize as migrate_data is
        # not provided in that case, leading to the directory not being removed
        # on the destination.
        if ((destroy_disks or is_shared_block_storage) or
            (CONF.workarounds.enable_libvirt_rbd_instance_dir_evac_cleanup and
             CONF.libvirt.images_type == 'rbd' and
             instance.host != CONF.host)):
