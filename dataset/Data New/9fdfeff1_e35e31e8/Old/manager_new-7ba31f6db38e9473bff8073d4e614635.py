...
class ComputeManager(manager.Manager):
    ...
    def _try_volume_connect(self, ctxt, instance, bdm, connector):
        """Updates the volume attachment for bdm and checks if the compute
        driver can connect the volume to this host.

        Called from prep_snapshot_based_resize_at_dest as part of validating
        a target host during a cross-cell resize operation.

        :param ctxt: nova auth request context
        :param instance: the instance being moved to this host
        :param bdm: BlockDeviceMapping object representing a volume attached
            to the instance
        :param connector: the volume connector dict for this host
        :return: True if the volume attachment can be updated and the volume
            can be connected to this host, False otherwise
        """
        # Conductor will have created an empty reserved attachment for this
        # BDM. We update it with the host connector which will attempt to
        # create an export in the storage backend and give us the
        # connection_info details to try and connect the volume on this host.
        # If any of this fails, conductor is responsible for cleaning up the
        # attachment.
        # TODO(mriedem): This might be wrong because we shouldn't update the
        # same volume attachment more than once but that's what we'd do if we
        # took this attachment and ran it through _prep_block_device during
        # finish_snapshot_based_resize_at_dest.
        attach_ref = self.volume_api.attachment_update(
            ctxt, bdm.attachment_id, connector, mountpoint=bdm.device_name)
        # Now try to connect this volume via the driver.
        return self.driver.can_connect_volume(
            ctxt, attach_ref['connection_info'], instance)

    # TODO(mriedem): Should we send error notifications (wrap_exception) and
    # record a fault (wrap_instance_fault) on pre-check failures? This is
    # really like a kind of post-scheduler validation and failures could be
    # "normal". Though prep_resize() does this as well so do we strive for
    # consistency?
    ...
    def prep_snapshot_based_resize_at_dest(
            ...
            bindings_by_port_id, volume_bdms):
        """Performs pre-cross-cell resize checks on the targeted destination
        ...
        Validates port bindings and volume attachments will work on the target
        host.

        This method should take care to rollback any resources it creates in
        case of failure.
        ...
        :param bindings_by_port_id: dict, keyed by port ID, of (inactive) port
            binding dicts for the destination host
        :param volume_bdms: BlockDeviceMappingList for the instance where each
            BDM in the list will have an attachment_id representing a
            "reserved" attachment to be connected to the target host
        ...
        try:
            ...
            # will create the MigrationContext object. Do the other validation
            # within the MoveClaim context so we can drop the claim if anything
            # fails.
            with self.rt.resize_claim(
                    ctxt, instance, flavor, nodename, migration,
                    image_meta=instance.image_meta, limits=limits):

                # NOTE(mriedem): We currently do not do anything with the port
                # bindings on the target host but bindings_by_port_id is passed
                # in case we need to in the future.

                # Validate storage for volumes by updating attachments against
                # this host and checking if we can connect them.
                connector = self.driver.get_volume_connector(instance)
                for bdm in volume_bdms:
                    # NOTE(mriedem): Do we really need to check that all
                    # volumes attached to the server need to be able to
                    # connect? Is one good enough? Or at least one per
                    # volume type?
                    can_connect = self._try_volume_connect(
                        ctxt, instance, bdm, connector)
                    if not can_connect:
                        raise exception.MigrationPreCheckError(
                            reason=(_('Unable to connect volume %s on '
                                      'this host.') % bdm.volume_id))

        except Exception as ex:
            err = six.text_type(ex)
            LOG.info('Cross-cell resize pre-checks failed for this host (%s). '
    ...
    def prep_snapshot_based_resize_at_source(
        ...
        the instance is not volume-backed, disconnecting volumes and unplugging
        VIFs.
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                # Move the resource allocations held by the migration consumer
                # on the source provider tree back to the instance consumer
                # and drop the allocations held by the instance on the
                # destination provider tree.
    ...
    def _prep_snapshot_based_resize_at_source(
        ...
        with self._error_out_instance_on_exception(
                ctxt, instance, instance_state=vm_states.STOPPED):
            ...
            # However, we do not call self.network_api.migrate_instance_start
            # since that would activate the destination host port bindings
            # created by conductor. We can wait to activate the destination
            # host port bindings when we finish the resize at the destination
            # host.
