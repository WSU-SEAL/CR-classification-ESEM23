...
class ServersViewBuilderTestV268(ServersViewBuilderTest):
    """Server ViewBuilder test for microversion 2.68

    The intent here is simply to verify that when showing server details
    after microversion 2.68 the response could have missing keys for those
    servers from the down cells.
    """
    wsgi_api_version = '2.68'

    def setUp(self):
        super(ServersViewBuilderTestV268, self).setUp()
        self.view_builder = views.servers.ViewBuilder()
        self.ctxt = context.RequestContext('fake', 'fake')

        def fake_is_supported(req, min_version="2.1", max_version="2.68"):
