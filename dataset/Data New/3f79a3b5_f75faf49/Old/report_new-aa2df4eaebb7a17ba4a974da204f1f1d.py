...
class SchedulerReportClient(object):
    ...
    def reset(self):
        """Invoked when SIGHUP is sent to the compute service.

        Currently used to clear the provider tree cache."""
        self._provider_tree = provider_tree.ProviderTree()
        self._association_refresh_time = {}
    ...
    def _ensure_resource_provider(self, context, uuid, name=None,
        ...
        # Optimization: If we already have the root provider in the cache, and
        # it's not stale, short out.
        if (self._provider_tree.exists(uuid)
    ...
    def get_provider_tree_and_ensure_root(self, context, rp_uuid, name=None,
        ...
        if name or self._associations_stale(rp_uuid):
    ...
    def update_from_provider_tree(self, context, new_tree, allocations=None):
        ...
        def catch_all(rp_uuid):
            ...
            try:
                ...
                # Invalidate the caches for the entire tree associated with the
                # failing provider (but leave other trees - e.g. sharing
                # providers - alone).
                try:
                    uuids = self._provider_tree.remove_tree(rp_uuid)
                    for uuid in uuids:
