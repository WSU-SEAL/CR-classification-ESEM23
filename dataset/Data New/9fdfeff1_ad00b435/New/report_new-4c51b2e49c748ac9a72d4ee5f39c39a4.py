...
class SchedulerReportClient(object):
    ...
    def remove_resources_from_instance_allocation(
        ...
        if not self._remove_resources_from_instance_allocation(
            ...
            error_reason = ("Cannot remove resources %s from the allocation "
                            "due to multiple subsequent generation conflict "
                            "in placement.")
            raise exception.AllocationUpdateFailed(
    ...
    def _remove_resources_from_instance_allocation(
        ...
        error_reason = "Cannot remove resources %s from allocation %s. %s"
        ...
        try:
                    ...
                    if allocation_on_rp['resources'][rc] < 0:
                        raise exception.AllocationUpdateFailed(
                            ...
                            error=error_reason % (
                                resources,
                                current_allocs,
                                'There is not enough inventory left on %s RP '
                                'to remove %d amount of %s resources.' % (
                                    rp_uuid, value, rc)))
            ...
            raise exception.AllocationUpdateFailed(
                ...
                error=error_reason % (
                    resources,
                    current_allocs,
                    'Key %s is missing from the allocation.' % e))
        ...
        if r.status_code != 204:
            ...
            if err['code'] == 'placement.concurrent_update':
                ...
                # works here as this function (re)query the allocations.
                raise Retry(
