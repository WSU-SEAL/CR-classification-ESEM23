...
rules = [
    ...
    policy.DocumentedRuleDefault(
        SERVERS % 'create:cell_down',
        base.RULE_ADMIN_API,
        """
This rule controls the compute API validation behavior of creating or resizing
a server when the requesting tenant already has other active servers in down
cells whose quota cannot be correctly obtained and would be skipped, thus later
leading to plausible overcommit scenario.

This rule defaults to ``rule:admin_api`` and hence only admins by default will
be able to create servers even if they have servers in the down cells at the
risk of wrong quota calculation/overcommitment on allowed quota. Normal users
will be blocked from server creation unless the rule is changed to allow
otherwise.
""",
        [
            {
