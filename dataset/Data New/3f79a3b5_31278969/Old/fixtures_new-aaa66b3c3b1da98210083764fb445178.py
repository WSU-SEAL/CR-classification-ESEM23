...
class DownCellFixture(fixtures.Fixture):
    ...
    a ``nova.test.TestingException`` per cell in the results. Therefore
    ...
    2. Using the fixture, list/show servers. Depending on the microversion
       used, the API should either return minimal results or return an error.
    ...
    def setUp(self):
        ...
        # NOTE(mriedem): We could make this more dynamic in the future by
        # allowing the caller to provide a list of cells to consider "down".

        def stub_scatter_gather_cells(ctxt, cell_mappings, *args, **kwargs):
            # Return a dict with an entry per cell mapping where the results
            # are some kind of exception.
            return {
                cell_mapping.uuid: test.TestingException()
