        ...
        with nova_context.target_cell(context, cell) as cctxt:
            ...
            for instance in instances:
                ...
                vif_list = VirtualInterfaceList.\
                    get_by_instance_uuid(cctxt, instance.get('uuid'))
                vif_list = [x for x in vif_list if not x.deleted]
                ...
                if cached_vif_ids == db_vif_ids:
                    # NOTE(mjozefcz): We need to bump the counter here.
                    # Otherwise if in batch of 50 instances there will be
                    # no migration the online_data_migration will not
                    # try next batch even there are still instances that
