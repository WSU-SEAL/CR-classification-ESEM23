...
class ServersSampleJson271Test(ServersSampleBase):
    ...
    def generalize_subs(self, subs, vanilla_regexes):
        subs['sg_uuid'] = ('[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}'
                   '-[0-9a-f]{4}-[0-9a-f]{12}')
        return subs
    ...
    def test_servers_post(self):
        ...
        subs['sg_uuid'] = self.sg_uuid
        name = self._get_request_name(False)
        response = self._do_post('servers', name, subs)
        status = self._verify_response('server-create-resp', subs,
                                        response, 202)
        return status
    ...
    def test_server_get_from_down_cells(self):
        ...
        subs = {'id': uuid}
        subs['sg_uuid'] = self.sg_uuid
        self._verify_response('server-get-server-group-resp',
                              subs, response, 200)
