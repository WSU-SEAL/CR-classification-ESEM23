    ...
    def _update_pci_request_spec_with_allocated_interface_name(
        ...
        for pci_request in instance.pci_requests.requests:
            if needs_update(
                ...
                dev_rp = self.reportclient._get_resource_provider(
                    context,
                    provider_uuids[0])
                if not dev_rp:
                    reason = (
                        'Resource provider %(provider)s used to allocate '
                        'resources for the pci request %(requester)s not '
                        'found in Placement.' %
                ...
                if len(rp_name_pieces) != 3:
                    reason = (
                        ...
                        'is <hostname>:<agentname>:<interfacename>' %
                        {'provider': dev_rp,
                         'requester': pci_request.requester_id})
                    raise exception.BuildAbortException(
                        instance_uuid=instance.uuid,
                ...
                for spec in pci_request.spec:
                    spec['pf_interface_name'] = rp_name_pieces[2]
                    modified = True
        if modified:
            instance.save()
