...
def resources_from_request_spec(ctxt, spec_obj):
    ...
    if target_host or target_node:
        hst_mng = host_manager.HostManager()
        nodes = hst_mng.get_compute_nodes_by_host_or_node(
            ctxt, target_host, target_node, target_cell)
        if not nodes:
            reason = (_('No such host - host: %(host)s node: %(node)s ') %
                      {'host': target_host, 'node': target_node})
            raise exception.NoValidHost(reason=reason)
        # ToDo(tetsuro): support multiple uuids in in_tree query
        if len(nodes) == 1:
            grp = res_req.get_request_group(None)
