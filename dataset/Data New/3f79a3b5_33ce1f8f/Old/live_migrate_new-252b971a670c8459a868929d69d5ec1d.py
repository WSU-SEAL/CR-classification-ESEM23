...
class LiveMigrationTask(base.TaskBase):
    ...
    def _check_can_migrate_pci(self):
        """Checks that an instance can migrate with PCI requests
        ATM support only if it contains VIF related PCI requests.
        """
        if self.instance.pci_requests is not None:
                ...
                if pci_request.alias_name is not None:
                    ...
                    raise exception.MigrationPreCheckError(
                        reason= "non VIF related PCI requests for instance "
                                "is not allowed in live migration.")
    ...
    def _bind_ports_on_destination(self, destination):
        ...
        try:
            ports_profile = {
                vif['id']:
                    self._get_updated_vif_profile(vif)
                for vif in self.instance.get_network_info()}
    ...
    def _get_updated_vif_profile(self, vif):
        if self.migrate_data.vifs:
            for migrate_vif in self.migrate_data.vifs:
                if migrate_vif.port_id == vif['id']:
                    return migrate_vif.profile
        return vif['profile']
