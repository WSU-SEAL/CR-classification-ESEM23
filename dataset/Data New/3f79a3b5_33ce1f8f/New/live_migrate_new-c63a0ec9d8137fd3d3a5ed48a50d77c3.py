...
class LiveMigrationTask(base.TaskBase):
    ...
    def _check_can_migrate_pci(self, src_host, dest_host):
        ...
        ATM support only if:
        Instance contains VIF related PCI requests,
        Neutron supports multiple port binding extension,
        Src and Dest host support VIF related PCI allocations.
        """
        if self.instance.pci_requests is not None and len(
                self.instance.pci_requests.requests) > 0:
    ...
    def _call_livem_checks_on_host(self, destination):
        ...
        if (self.network_api.supports_port_binding_extension(self.context) and
            ...
            if not hasattr(self.migrate_data, 'vifs'):
    ...
    def _bind_ports_on_destination(self, destination):
        ...
        try:
            ...
            migrate_vifs_with_profile = [mig_vif for mig_vif in
                                         self.migrate_data.vifs
                                         if hasattr(mig_vif, 'profile')]
            ports_profile = {
                mig_vif.port_id: mig_vif.profile
                for mig_vif in migrate_vifs_with_profile} if len(
                migrate_vifs_with_profile) else None
