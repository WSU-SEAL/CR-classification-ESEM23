...
removed during cleanup on hosts using the RBD imagebackend. This avoids the
following bugs with evacuation and revert resize clean up that lead to the
instance directory remaining on the host. This will in turn result in
DestinationDiskExists errors being raised if the instances ever attempt to
return to these hosts:
...
.. note:: Operators will need to ensure that the instance directory itself is
  ...
  console.log, kernels, ramdisks and any additional files being used by the
  running instance will be lost.

.. note:: Additionally hard rebooting an instance with this workaround enabled
  will result in the console.log being truncated. Also any kernels or ramdisks
  provided to the instance as image properties will be recreated if still
  available within the image service.
