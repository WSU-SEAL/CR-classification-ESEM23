...
class UtimeWorkaroundTestCase(test.NoDBTestCase):
    ...
    def test_update_utime_eio(self):
        ...
        # self.assertRaises() only allows you to assert the class of the error,
        # not its properties.
        try:
            imagebackend._update_utime_ignore_eacces(mock.sentinel.path)
        except Exception as raised:
            self.assertIs(raised, e)
        else:
            self.fail('_update_utime_ignore_eacces did not raise an exception')
