...
class PciDevTracker(object):
    ...
    def _free_instance(self,
                       instance,
                       dev_status_to_free=(fields.PciDeviceStatus.CLAIMED,
                                           fields.PciDeviceStatus.ALLOCATED)):
        """Free device that are in device_status_to_free status for instance.

        :param instance: instance object
        :param dev_status_to_free: list of expected device statuses
        which this method will free.
        :return:
        """
        ...
        for dev in self.pci_devs:
            if dev.status in dev_status_to_free:
    ...
    def free_instance_allocations(self, context, instance):
        if self.allocations.pop(instance['uuid'], None):
            self._free_instance(instance, fields.PciDeviceStatus.ALLOCATED)

    def free_instance_claims(self, context, instance):
        if self.claims.pop(instance['uuid'], None):
            self._free_instance(instance, fields.PciDeviceStatus.CLAIMED)
