...
class ServersSampleJson268Test(ServersSampleBase):
    microversion = '2.70'
    scenarios = [('v2_70', {'api_major_version': 'v2.1'})]

    def setUp(self):
        super(ServersSampleJson268Test, self).setUp()
        self.common_subs = {
            'hostid': '[a-f0-9]+',
            'instance_name': 'instance-\d{8}',
            'hypervisor_hostname': r'[\w\.\-]+',
    ...
    def test_servers_get_server_group(self):
        uuid = self.test_servers_post()
        self.stub_out(
    ...
    def test_servers_update_server_groups(self):
        uuid = self.test_servers_post()
        self.stub_out(
            'nova.objects.InstanceGroup.get_by_instance_uuid',
             self.stub_get_by_instance_uuid)
