...
class InstanceMappingTestCase(test.NoDBTestCase):
    ...
    def test_populate_user_id(self):
        ...
        # Only four left were fixable and the fifth instance found has no
        # cell and cannot be migrated yet.
        ...
        # We expect 12 of them to have user_id set after migration (13 total,
        # with the not-yet-scheduled instance ignored).
        # We need to load the raw database objects instead of versioned objects
        # because InstanceMapping.user_id is not loadable when NULL.
        ims = instance_mapping.InstanceMappingList._get_by_project_id_from_db(
            self.context, self.context.project_id)
        self.assertEqual(12, len(
            [im for im in ims if im['user_id'] is not None]))
...
class TestInstanceMappingUserId(test.TestCase):
    def setUp(self):
        super(TestInstanceMappingUserId, self).setUp()
        # Use the same user and project as the sample mappings.
    ...
    def test_user_id_not_set_if_null_from_db(self):
        ...
        im = instance_mapping.InstanceMapping.get_by_instance_uuid(
            self.ctxt, db_mapping['instance_uuid'])
