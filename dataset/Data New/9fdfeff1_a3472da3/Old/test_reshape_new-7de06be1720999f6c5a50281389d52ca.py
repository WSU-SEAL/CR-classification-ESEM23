...
class VGPUReshapeTests(base.ServersTestBase):
    ...
    def test_create_servers_with_vgpu(self):
        ...
        # { inst.uuid: pgpu_name }
        inst_to_pgpu = {}
        # TODO(efried): How do I get Instance from serverN?
        for server in (server1, server2):
            mock_inst = mock.Mock()
            mock_inst.configure_mock(uuid=server['id'], name=server['name'])
            mdevs = self.compute.driver._get_all_assigned_mediated_devices(
                mock_inst)
            # Fail unless exactly one mdev
            mdev_uuid, = list(mdevs)
            dev_name = "mdev_" + mdev_uuid.replace('-', '_')
            mdev_info = self.compute.driver._get_mediated_device_information(
                dev_name)
            inst_to_pgpu[server['id']] = mdev_info['parent']
