...
class ViewBuilder(common.ViewBuilder):
    ...
    def basic(self, request, instance, show_extra_specs=False,
        ...
        if cell_down_support and 'display_name' not in instance:
            # NOTE(tssurya): If the microversion is >= 2.68, this boolean will
    ...
    def show(self, request, instance, extend_address=True,
        ...
        if cell_down_support and 'display_name' not in instance:
            ...
            if 'flavor' in instance:
                # If the key 'flavor' is present for an instance from a
                # down cell, it means that the request is a
                # ``GET /servers{server_id}`` and thus we include the
                # information from the request_spec of the instance like
                # its flavor, image, avz, and user_id in addition to the
                # basic information from its instance_mapping.
                avz = (instance.availability_zone
                       if instance.availability_zone is not None
                       else "UNKNOWN")
                context = request.environ['nova.context']
                show_extra_specs = context.can(
                    fes_policies.POLICY_ROOT % 'index', fatal=False)
                ret["server"]["user_id"] = instance.get("user_id") or ""
    ...
    def _get_flavor(self, request, instance, show_extra_specs,
        ...
        if cell_down_support:
            # NOTE(tssurya): From microversion 2.68 we handle down cells for
            # which we may get partial constructs with certain keys like
            # ``flavor`` for the ``GET /servers{server_id}`` request for the
            # servers from the down cells. In such cases we just return the
            # value from the request_spec of the instance instead of querying
            # instance.flavor from the cell database since the cell will be
            # down.
