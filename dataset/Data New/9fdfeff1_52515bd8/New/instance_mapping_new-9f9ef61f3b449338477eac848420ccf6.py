...
def populate_user_id(context, max_count):
    ...
    ims = (
        ...
        .limit(max_count).all())
    found = len(ims)
    ims_by_inst_uuid = {im.instance_uuid: im for im in ims}
    ...
    for cell_id, inst_uuids in inst_uuids_by_cell_id.items():
        ...
        # and update it.
        for instance in instances:
            # Ignore any instance record that might have user_id=None (example:
            # marker instance for fill_virtual_interface_list data migration),
            # else we could get into an infinite migration loop.
            if instance.user_id is None:
                found -= 1
                continue
            # Ignore any 'deleted' instance because they cannot be restored
            # (only soft-deleted instances can be restored).
            if instance.deleted:
                found -= 1
                continue
            im = ims_by_inst_uuid[instance.uuid]
