...
class ServersControllerTestV270(ControllerTest):
    wsgi_api_version = '2.70'

    class fake_sg(object):
            uuid = 'fake_uuid'
    ...
    def test_show_with_server_group(self):
        req = self.req('/fake/servers/%s' % FAKE_UUID)
        with mock.patch.object(objects.InstanceGroup,
                                'get_by_instance_uuid') as mock_getgroup:
            mock_getgroup.return_value = self.fake_sg()
            res_dict = self.controller.show(req, FAKE_UUID)

        self.assertEqual(self.expect_sg, res_dict['server']['server_groups'])
...
class ServersControllerRebuildTestV270(ControllerTest):
    class fake_sg(object):
            uuid = 'fake_uuid'
    ...
    def setUp(self):
        super(ServersControllerRebuildTestV270, self).setUp()
        self.req = fakes.HTTPRequest.blank('/fake/servers/a/action',
                                           use_admin_context=True)
        self.req.method = 'POST'
        self.req.headers["content-type"] = "application/json"
        ...
        self.req_project_id = self.req.environ['nova.context'].project_id
        self.req.api_version_request = \
            api_version_request.APIVersionRequest('2.70')
        self.body = {
            "rebuild": {
                "imageRef": self.image_uuid,
                "user_data": None
    ...
    def test_rebuild_with_server_group(self):
        with mock.patch.object(objects.InstanceGroup,
                                'get_by_instance_uuid') as mock_getgroup:
            mock_getgroup.return_value = self.fake_sg()
            self.req.body = jsonutils.dump_as_bytes(self.body)
            server = self._rebuild_server()
        self.assertEqual(self.expect_sg, server['server_groups'])
...
class ServersControllerUpdateTestV270(ServersControllerUpdateTest):
    class fake_sg(object):
            uuid = 'fake_uuid'
    ...
    def setUp(self):
        super(ServersControllerUpdateTestV270, self).setUp()

    def _get_request(self, body=None):
        req = super(ServersControllerUpdateTestV270, self)._get_request(
            body=body)
        req.api_version_request = api_version_request.APIVersionRequest('2.70')
        return req

    def test_update_with_server_group(self):
        body = {'server': {
                  'name': 'server_test',
               }}
        with mock.patch.object(objects.InstanceGroup,
