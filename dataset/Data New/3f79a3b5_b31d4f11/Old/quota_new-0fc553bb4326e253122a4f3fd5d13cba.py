...
class QuotaEngine(object):
    ...
    def count_as_dict(self, context, resource, cell_down_support=False,
                      *args, **kwargs):
        ...
        return res.count_as_dict(context, cell_down_support, *args, **kwargs)
...
def _instances_cores_ram_count(context, cell_down_support, project_id, user_id=None,):
    ...
    """
    # TODO(melwitt): In the future, we should query placement for cores/ram
    # and InstanceMappings for instances.
    ...
    # as a part of the handling-down-cell blueprint, we
    # also introduce a policy check in the new microversion for (dis-)allowing
    # server creation if the tenant already has active instances in the down
    # cells.
    if CONF.api.instance_list_per_project_cells:
        ...
        results = nova_context.scatter_gather_all_cells(
            context, objects.InstanceList.get_counts,
    ...
    for cell_uuid, result in results.items():
        if not nova_context.is_cell_failure_sentinel(result):
            ...
            # TODO(mriedem): Optimize this policy check to only do it once
            # in the for loop in case there are multiple down cells.
            context.can(servers_policies.SERVERS % 'create:cell_down')
    return total_counts
