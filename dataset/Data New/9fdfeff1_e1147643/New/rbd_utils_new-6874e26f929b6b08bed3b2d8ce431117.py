...
class RBDDriver(object):
    ...
    def import_image(self, base, name, fmt):
        """Import RBD volume from image file.

        :base: Path to image file
        :name: Name of RBD volume
        :fmt: Format of image file
        """
        # RBD volume doesn't support QCOW2 for hosting a virtual machine disk.
        # Thus if you want to boot virtual machines in RBD volume, the image
        # format must be RAW.
        # If you set the force_raw_images to False and the glance image format
        # is QCOW2, the format of image file here is QCOW2.
        # So that, get and check the format of image file. Then choose the
        # right way to import rbd volume from image file.
        if fmt in ['raw', 'iso']:
