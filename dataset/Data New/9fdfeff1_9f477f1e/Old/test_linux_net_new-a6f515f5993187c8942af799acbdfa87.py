...
class LinuxNetworkTestCase(test.NoDBTestCase):
    ...
    @mock.patch('nova.privsep.linux_net.routes_show',
                return_value=('fake', 0))
    @mock.patch('nova.privsep.linux_net.lookup_ip', return_value=('fake', ''))
    ...
    @mock.patch('nova.privsep.linux_net.address_command_horrid')
    ...
    def _test_initialize_gateway(self, existing, expected,
                                 mock_lookup_ip, mock_change_ip,
                                 mock_address_command, mock_route_add,
                                 mock_route_delete, mock_routes, mock_execute,
                                 routes='', routes_show_called=True,
                                 deleted_routes=None, added_routes=None,
                                 horrid_commands=None, changed_ip=None):
        self.flags(fake_network=False)
        executes = []
        ...
        if horrid_commands:
            mock_address_command.assert_has_calls(horrid_commands)
        if changed_ip:
            mock_change_ip.assert_has_calls(changed_ip)
        ...
        self._test_initialize_gateway(
            ...
            horrid_commands=[
                ...
                mock.call('eth0', 'add',
                          ['192.168.0.1/24', 'brd', '192.168.0.255',
                           'scope', 'global'])],
            changed_ip=[mock.call('eth0', '2001:db8::/64')]
            )
    ...
    def test_initialize_gateway_no_move_right_ip(self):
        ...
        self._test_initialize_gateway(
            existing, expected, routes_show_called=False,
            changed_ip=[mock.call('eth0', '2001:db8::/64')])
    ...
    def test_ensure_bridge_brings_up_interface(self):
        ...
        with test.nested(
            ...
            mock.patch.object(netifaces, 'ifaddresses')
              ...
              lookup_ip, _execute, ifaddresses):
