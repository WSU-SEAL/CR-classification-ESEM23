...
def get_weight_multiplier(host_state, type):
    multiplier_name = '%s_weight_multiplier' % type
    if host_state and host_state.aggregates:
        cpu_multipliers = []
        for agg in host_state.aggregates:
            val = agg.metadata.get(multiplier_name, None)
            try:
                cpu_multipliers.append(float(val))
            except (ValueError, TypeError):
                LOG.error("Failed to load %(multiplier_name)s, the value "
                          "%(val)s is invalid",
                          {'multiplier_name': multiplier_name, 'val': val})
        if cpu_multipliers:
            return min(cpu_multipliers)
    return getattr(CONF.filter_scheduler, multiplier_name)
