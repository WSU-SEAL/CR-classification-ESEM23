...
class ProviderTree(object):
    ...
    def get_provider_uuids(self, name_or_uuid=None, whole_tree=False):
        ...
        :param name_or_uuid: Provider name or UUID representing the root of a
                             ...
                             ProviderTree.
        :param whole_tree: Ignored unless name_or_uuid is specified.
                           If False, the method returns UUIDs of only
                           descendants of the provider specified by
                           name_or_uuid; otherwise it returns UUIDs of all the
                           providers in the associated tree (but not other
                           trees in the ProviderTree object).
        """
        if name_or_uuid is not None:
            with self.lock:
                return self._find_with_lock(
                    name_or_uuid, return_root=whole_tree).get_provider_uuids()
