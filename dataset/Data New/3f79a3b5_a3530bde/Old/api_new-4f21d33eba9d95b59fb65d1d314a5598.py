...
from oslo_limit import limit as ol
...
class API(base.Base):
    ...
    def _check_injected_file_quota(self, context, injected_files):
        ...
        try:
            ...
            limit.check_api_limit(injected_files=len(injected_files))
        ...
        try:
            ...
            limit.check_api_limit(injected_file_path_bytes=max_path,
                                  injected_file_content_bytes=max_content)
    ...
    def _check_metadata_properties_quota(self, context, metadata=None):
        ...
        try:
            ...
            limit.check_api_limit(metadata_items=num_metadata)
        except exception.OverQuota as exc:
                ...
                if instance_group:
                    if check_server_group_quota:
                        try:
                            ...
                            limit.check_static_limit(context,
                                'server_group_members',
                                instance_group,
                                additional_resource=1)
                        except exception.OverQuota:
    ...
    def _check_quota_for_upsize(context, instance, current_flavor, new_flavor):
        ...
        if deltas:
            try:
                ...
                # TODO(johngarbutt) can we make this atomic with placement
                # allocations somehow?
                limit.check_dynamic_limits(context, project_id, deltas)
            except exception.OverQuota as exc:
    ...
    def resize(self, context, instance, flavor_id=None, clean_shutdown=True,
        ...
        # ensure there is sufficient headroom from upsizes
        if flavor_id:
            project_id, user_id = quotas_obj.ids_from_instance(
                context, instance)

            # Based on the usage of new_instance_type, it appears to be
            # a flavor definition of the new specs. Let's use it to build a set
            # of claims.
            cores_claim = ol.Claim('cores', new_instance_type.get('cores', 0))
            ram_claim = ol.Claim('ram_mb', new_instance_type.get('ram', 0))
            claims = [cores_claim, ram_claim]

            callback = limit.calculate_resource_usage_for_project
            enforcer = os.Enforcer(claims, project_id, callback)

            # NOTE: If, or when, the retry logic ever makes its way back into
            # the nova-api from nova-conductor, we have an opportunity to
            # simplify this by using a context manager to perform the retry
            # check in an implementation of  __exit__(). For now, we should be
            # able to invoke retry checks to prevent race conditions without
            # needing to rely on a context manager since the initial check and
            # the retry check are happening in different processes.
            enforcer.enforce()
...
class KeypairAPI(base.Base):
    ...
    def _validate_new_key_pair(self, context, user_id, key_name, key_type):
        ...
        try:
            ...
            limit.check_static_limit(context, 'key_pairs', user_id, 1)
        except exception.OverQuota:
