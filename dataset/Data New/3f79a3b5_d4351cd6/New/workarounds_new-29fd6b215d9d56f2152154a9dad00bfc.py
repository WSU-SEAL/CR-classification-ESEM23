...
"""),
    cfg.BoolOpt(
        'enable_libvirt_rbd_instance_dir_evac_cleanup',
        default=False,
        help="""
Cleanup the instance directory during evacuation of instances using rbd images.

When enabled this workaround will ensure that the instance directory is removed
during the cleanup of an evacuation when using the RBD imagebackend to host the
instance disks. This will allow the instance to return to the local host in
future without encountering DestinationDiskExists errors as detailed in the
...
.. note:: Operators will need to ensure that the instance directory itself is
  ...
  console.log and any additional files present in the instance directory will
  be lost.
