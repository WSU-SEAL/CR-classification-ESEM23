    ...
    def _set_hvdevs(self, devices):
        ...
        for existed in self.pci_devs:
            if existed.address in exist_addrs - new_addrs:
                ...
                # nolonger reported by the hypervisor or have been removed
                ...
                try:
                    ...
                    # this host and the resouce track assinging the pci device
                    # to a second vm. To prevent this bug we skip deleting the
                    # device from the db in this iteration and will try again
                    # on the next sync.
                    continue
                else:
