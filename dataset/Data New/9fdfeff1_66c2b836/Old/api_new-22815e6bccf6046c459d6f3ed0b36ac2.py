...
class API(base.Base):
    ...
    @staticmethod
    def _update_queued_for_deletion_and_user_id(context, instance, qfd):
        ...
        if not qfd and im.user_id is None:
            # If we're un-deleting an instance (soft-delete restore), go ahead
            # and migrate the user_id if it's not yet migrated.
            # TODO(melwitt): This setting of user_id can be removed in the
            # T release, when all migrations are complete.
    ...
    def _do_delete(self, context, instance, bdms, local=False):
        if local:
            ...
            self.compute_rpcapi.terminate_instance(context, instance, bdms,
                                                   delete_type='delete')
        self._update_queued_for_deletion_and_user_id(context, instance, True)
    ...
    def restore(self, context, instance):
        ...
        if instance.host:
            ...
            instance.deleted_at = None
            instance.save(expected_task_state=[None])
        self._update_queued_for_deletion_and_user_id(context, instance, False)
    ...
    def _save_user_id_in_instance_mapping(mapping, instance):
        ...
        # in the T release, when all migrations are complete.
