...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _get_existing_mdevs_not_assigned(self, requested_types=None,
        ...
        available_mdevs = set()
        for mdev in mdevs:
            if (parent is not None and mdev['parent'] == parent
                    or not parent):
    ...
    def _allocate_mdevs(self, allocations):
        ...
        # TODO(sbauza): WE NEED TO FIND A WAY TO MAP THE RP UUID WITH ITS NAME
        # FIXME(sbauza): As we don't ask for specific mdevs, if the allocation
        # is related to a specific pGPU, we won't use it but the resources
        # will still be OK until we support multiple types
        # For the moment, only pretend it's the root RP so we don't need to
        # ask for specific mdevs. Let's be explicit with the attribute value
    ...
    def _is_reshape_needed_vgpu_on_root(provider_tree, nodename):
        ...
        return rc_fields.ResourceClass.VGPU in root_node.inventory
    ...
    def _ensure_pgpu_providers(inventories_dict, provider_tree, nodename):
        ...
        for pgpu_dev_id, inventory in inventories_dict.items():
            ...
            pgpu_inventory = {rc_fields.ResourceClass.VGPU: inventory}
    ...
    def _reshape_vgpu_allocations(
        ...
        self._check_vgpu_allocations_match_real_use(
            vgpu_count_per_pgpu, resources[rc_fields.ResourceClass.VGPU],
        ...
        for pgpu_dev_id, pgpu_rp in pgpu_rps.items():
            ...
            if vgpu_count:
                allocs[pgpu_rp.uuid] = {
                    'resources': {
                        rc_fields.ResourceClass.VGPU: vgpu_count
                    }
                }
        # And remove the VGPU allocation from the root node provider.
        del resources[rc_fields.ResourceClass.VGPU]
    ...
    def _reshape_gpu_resources(
        ...
        for consumer_uuid, alloc_data in allocations.items():
            ...
            for rp_uuid in list(allocs):
                ...
                if rc_fields.ResourceClass.VGPU in resources:
    ...
    def _update_provider_tree_for_vgpu(self, inventories_dict, provider_tree,
        ...
        if self._is_reshape_needed_vgpu_on_root(provider_tree, nodename):
            ...
            if rc_fields.ResourceClass.VGPU in root_node.inventory:
                del root_node.inventory[rc_fields.ResourceClass.VGPU]
