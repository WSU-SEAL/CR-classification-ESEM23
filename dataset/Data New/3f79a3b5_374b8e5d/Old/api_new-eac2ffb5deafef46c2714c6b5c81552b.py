...
class API(base.Base):
    ...
    def __init__(self, image_api=None, network_api=None, volume_api=None,
        ...
        self.placementclient = None  # Loaded in _get_placement_client().
    ...
    def _get_placement_client(self):
        if self.placementclient is None:
            self.placementclient = reportclient.SchedulerReportClient()
    ...
    def _local_delete(self, context, instance, bdms, delete_type, cb):
        ...
        with compute_utils.notify_about_instance_delete(
            ...
            # compute service.
            self._get_placement_client().delete_allocation_for_instance(
                context, instance.uuid)
...
class AggregateAPI(base.Base):
    ...
    def __init__(self, **kwargs):
        ...
        self.placement_client = None  # Loaded in _get_placement_client().
        super(AggregateAPI, self).__init__(**kwargs)

    def _get_placement_client(self):
        if self.placement_client is None:
            self.placement_client = reportclient.SchedulerReportClient()
        return self.placement_client
    ...
    def add_host_to_aggregate(self, context, aggregate_id, host_name):
        ...
        self.scheduler_client.update_aggregates(context, [aggregate])
        try:
            self._get_placement_client().aggregate_add_host(
    ...
    def remove_host_from_aggregate(self, context, aggregate_id, host_name):
        ...
        self.scheduler_client.update_aggregates(context, [aggregate])
        try:
            self._get_placement_client().aggregate_remove_host(
                context, aggregate.uuid, host_name)
        except exception.PlacementAPIConnectFailure:
            # NOTE(jaypipes): Rocky should be able to tolerate the nova-api
            # service not communicating with the Placement API, so just log a
