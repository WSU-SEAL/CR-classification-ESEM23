...
# Note(sean-k-mooney): i would ideally default to PREFERRED
# policy to allow more candiate host for migration however
# currently you cannot specify a numa affinity policy for
# neutron sriov ports so they default to the legacy policy
# which enforce numa affinity if the device exposes a numa
# affinity.
NUMA_POLICY_DEFAULT = obj_fields.PCINUMAAffinityPolicy.LEGACY
VF_TYPE = obj_fields.PciDeviceType.SRIOV_VF
PF_TYPE = obj_fields.PciDeviceType.SRIOV_PF


def get_pci_request_from_vif(vif, numa_policy=NUMA_POLICY_DEFAULT):
    """build pci request from VIF

       :param vif: network VIF model objects
       :param numa_policy: desired PCI NUMA affinity policy
       :returns: PCI request Object or None
    """
    ...
    if vnic_type in network_model.VNIC_TYPES_SRIOV:
        spec["dev_type"] = (PF_TYPE if vnic_type ==
            network_model.VNIC_TYPE_DIRECT_PHYSICAL else VF_TYPE)
        spec["physical_network"] = vif['profile']['physical_network']
        pci_request = objects.InstancePCIRequest(
            count=1,
            spec=[spec],
...
def get_instance_pci_request_from_vif(instance, vif):
    """Given an Instance return the PCI request associated
    with the given VIF (if any).

    In this method we assume a VIF is associated with a PCI device
    iff 'pci_slot' attribute exists in the vif 'profile' dict.

    :param instance: instance object
    :param vif: network VIF model object
    :return: instance's PCIRequest object associated with the given VIF or None
    """

    vif_pci_dev_address = None
    vif_profile = vif.get('profile', None)
    if vif_profile:
        vif_pci_dev_address = vif_profile.get('pci_slot', None)
    vif_pci_request = None
    if vif_pci_dev_address:
        # Find PCIDevice associated with vif_pci_dev_address
        vif_pci_dev = None
        for pci_dev in instance.pci_devices:
            if pci_dev.address == vif_pci_dev_address:
                vif_pci_dev = pci_dev
        if vif_pci_dev:
            # Find PCIRequest associated with the given PCIDevice in instance
            for pci_request in instance.pci_requests.requests:
                if pci_request.request_id == vif_pci_dev.request_id:
                    vif_pci_request = pci_request
        if vif_pci_request is None:
            LOG.error("Failed to locate PCI request associated with"
                      " the given VIF PCI address: %s", vif_pci_dev_address)
    return vif_pci_request
