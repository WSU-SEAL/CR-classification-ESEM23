...
class API(base.Base):
    ...
    def _validate_flavor_image(self, context, image_id, image,
        ...
        :param instance_type: Flavor object (generally accessed as dict)
        :param root_bdm: BlockDeviceMapping for root disk.  Will be None for
               the resize case as the disk does not change.
        :param validate_numa: Flag to indicate whether or not to validate
               the NUMA-related metadata.  In the instance creation code path
               this is handled separately and so we can skip it here.
