...
class ComputeManager(manager.Manager):
    ...
    def _default_block_device_names(self, instance, image_meta, block_devices):
        ...
        # NOTE(melwitt): This is where we can raise TooManyDiskDevices if
        # we exceed the configured maximum allowed, via
        # default_device_names_for_instance. This runs before any of the lower
        # level attach code, which is where we create volumes (if we're
        # booting from an image-backed volume). So, this means we will not
        # create any volumes if we exceed the configured maximum number of
        # disk devices allowed to attach to a single instance.
    ...
    def _prep_block_device(self, context, instance, bdms):
        ...
        try:
            # NOTE(melwitt): This is where we can raise TooManyDiskDevices if
            # we exceed the configured maximum allowed, via
            # get_device_name_for_instance. This runs before any of the lower
            # level attach code, which is where we create volumes (if we're
            # booting from an image-backed volume). So, this means we will not
            # create any volumes if we exceed the configured maximum number of
            # disk devices allowed to attach to a single instance.
    ...
    def _build_resources(self, context, instance, requested_networks,
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                ...
                exception.TooManyDiskDevices) as e:
            # Make sure the async call finishes
            if network_info is not None:
                network_info.wait(do_raise=False)
    ...
    def reserve_block_device_name(self, context, instance, device,
        ...
        def do_reserve():
            ...
            # NOTE(melwitt): This is where we will raise TooManyDiskDevices if
            # we exceed the configured maximum allowed, via
            # get_device_name_for_instance. The reserve_block_device_name
            # method is a synchronous RPC call from the API as part of the
            # attach_volume flow, so if we exceed the configured maximum here,
            # we will fail fast in the API.
