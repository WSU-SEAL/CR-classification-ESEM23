...
from nova.objects import instance_pci_requests as instance_pci_requests_obj
...
class ComputeManager(manager.Manager):
    ...
    def check_can_live_migrate_destination(self, ctxt, instance,
        ...
        try:
            ...
            port_id_to_pci = self._claim_pci_for_instnace_vifs(ctxt, instance)
    ...
    def _post_live_migration(self, ctxt, instance,
        ...
        # destination node
        self._get_resource_tracker().pci_tracker.free_instance_allocations(
            ctxt,
            instance)
    ...
    def post_live_migration_at_destination(self, context, instance,
        ...
        pci_tracker = self._get_resource_tracker().pci_tracker
        if pci_tracker:
            pci_tracker.allocate_instance(instance)
            # Note(adrianc): Update DB with new allocations.
            # do not rely on resource_tracker periodic job
            # TODO(adrianc): do we need to protect save operation with
            # COMPUTE_RESOURCE_SEMAPHORE ?
            pci_tracker.save(context)
    ...
    def rollback_live_migration_at_destination(self, context, instance,
        ...
        try:
            ...
            self._get_resource_tracker().pci_tracker.free_instance_claims(
                context,
                instance)
    ...
    def _claim_pci_for_instnace_vifs(self, ctxt, instance):
        ...
        for vif in instance.get_network_info():
            pci_req = pci_request.get_instance_pci_request_from_vif(ctxt,
                                                                    instance,
                                                                    vif)
            if pci_req:
                pci_req_id_to_port_id[pci_req.request_id] = vif['id']
                pci_req_lst.append(pci_req)

        if len(pci_req_lst) > 0:
            # Create PCI requests and claim against PCI resource tracker
            # Note(adrianc): we Claim against the same requests as on the
            # source node.
            vif_pci_requests = instance_pci_requests_obj.InstancePCIRequests(
                requests=pci_req_lst,
                instance_uuid=instance.uuid)
            pci_tracker = self._get_resource_tracker().pci_tracker
            if pci_tracker:
                claimed_pci_devices_objs = pci_tracker.claim_instance(
                    ctxt, vif_pci_requests, None)
