...
class ResourceTracker(object):
    ...
    def _update_available_resource(self, context, resources, startup=False):
        ...
        # By clearing the report client's cache for this node, we force _update
        # to go looking for its provider record in placement, creating it if
        # it's not already there. Otherwise _update could skip querying
        # placement for performance reasons.
        if is_new_compute_node:
            self.scheduler_client.reportclient.clear_provider_cache_for_tree(
                cn.uuid)
