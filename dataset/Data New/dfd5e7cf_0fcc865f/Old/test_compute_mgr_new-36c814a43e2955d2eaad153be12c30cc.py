...
class ComputeManagerUnitTestCase(test.NoDBTestCase):
    ...
    def _test_check_can_live_migrate_destination(self, do_raise=False):
        ...
        with test.nested(
            ...
            mock.patch.object(self.compute, '_claim_pci_for_instnace_vifs'),
            mock.patch.object(self.compute,
...
class ComputeManagerMigrationTestCase(test.NoDBTestCase):
    ...
    def test_post_live_migration_at_destination_success(self):
        ...
        def _do_test(mock_notify, post_live_migration_at_destination,
            ...
            _get_compute_info.assert_called_once_with(self.context,
                                                      self.compute.host)
            rt_mock.pci_tracker.allocate_instance.assert_called_once_with(
                self.instance)
    ...
    def _call_post_live_migration(self, mock_clean, *args, **kwargs):
        ...
        @mock.patch.object(self.compute, 'network_api')
        @mock.patch('nova.objects.BlockDeviceMappingList.get_by_instance_uuid')
        def _do_call(bdm, nwapi, notify, rpc, update, get_rt):
            get_rt.return_value = mock_rt
            result = self.compute._post_live_migration(self.context,
                                                       self.instance,
                                                       'foo',
                                                       *args, **kwargs)
            return result
        ...
        mock_rt.pci_tracker.free_instance_allocations.assert_called_once_with(
            self.context,
            self.instance)
        return result
    ...
    def test_rollback_live_migration_at_destination_port_binding_delete_fails(
        ...
        def _do_test(driver_rollback, get_bdms, network_api, legacy_notify,
            ...
            driver_rollback.assert_called_once_with(
                ...
                migrate_data=mock.sentinel.migrate_data)
            rt_mock.pci_tracker.free_instance_claims.assert_called_once_with(
                self.context,
                self.instance)
    ...
    def test__claim_pci_for_instance_vifs(self):
        ...
        def _test(mock_get_network_info,
                  ...
                  mock_get_resource_tracker):
            mock_rt = mock.Mock()
            mock_get_resource_tracker.return_value = mock_rt
            # when no VIFS, expecting no pci devices to be claimed
            mock_get_network_info.return_value = []
            port_id_to_pci = self.compute._claim_pci_for_instnace_vifs(
                self.context,
                self.instance)
            mock_rt.pci_tracker.claim_instance.assert_not_called()
            ...
            nw_vifs = network_model.NetworkInfo(
                [network_model.VIF(
                    ...
                    profile={'pci_slot': 'gotham',
                             'pci_vendor_info': 'dc:batman'}),
                    network_model.VIF(
                        id=uuids.port1,
                        vnic_type='normal',
            ...
            claimed_pci_dev = objects.PciDevice(request_id=uuids.pci_req,
                                                ...
                                                vendor_id="marvel",
                                                product_id="iron-man")
            mock_rt.pci_tracker.claim_instance.return_value = [claimed_pci_dev]
            ...
            self.assertTrue(mock_rt.pci_tracker.claim_instance.called)
            self.assertEqual(len(port_id_to_pci), 1)

        _test()

    def test__update_migrate_vifs_profile_with_pci(self):
        # define two migrate vifs with only one pci to update
        ...
        self.compute._update_migrate_vifs_profile_with_pci(mig_vifs,
                                                           port_id_to_pci_dev)
        # make sure migrate vifs porfile was updated.
