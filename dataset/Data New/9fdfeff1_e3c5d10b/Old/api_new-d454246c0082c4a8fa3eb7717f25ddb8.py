...
class API(base.Base):
    ...
    def _get_instance_from_cell(self, context, im, expected_attrs,
        ...
        if not nova_context.is_cell_failure_sentinel(result[cell_uuid]):
            ...
            try:
                ...
                avz = (rs.availability_zone
                       if rs.availability_zone is not None else "UNKNOWN")
    ...
    def get_all(self, context, search_opts=None, limit=None, marker=None,
        ...
        for opt, value in search_opts.items():
            if opt == 'all_tenants':
                # NOTE(tssurya): This must be the cell_down_support case.
                # Hence we don't pass this to the below layers.
                continue
            # Do remappings.
        ...
        if cell_down_support:
            ...
            if 'all_tenants' in search_opts:
                ...
                # is when using "--all-tenants" in which case we do not want
                # the query to be restricted based on the project_id.
                project = None
                # Since we skipped popping this in the top layer, we pop this
                # out now since its purpose is over.
